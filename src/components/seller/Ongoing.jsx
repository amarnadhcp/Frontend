...
export default function Ongoing() {
  ...
  const { isLoading, error, data } = useQuery({
    queryKey: ["ongoing"],
    queryFn: () =>
      userRequest.get("freelancer/ongoing").then((res) => res.data),
  });

  const handelAccept = async (proposalId) => {
    await WorkCompleted(proposalId);
            ...
            <tr key={proposal._id}>
                ...
                <div className="flex items-center">
                  <div className="ml-3">
                    <img
                      ...
                      src={proposal.buyerId.img || "https://res.cloudinary.com/dvprhxg7x/image/upload/v1692803989/asset/noavatar_vhrf74.jpg"}
                      alt="category image"
                    />
                  </div>
                </div>
              ...
              <td className="px-3 sm:px-5 py-4 sm:py-5 border-b border-gray-200 bg-white text-xs sm:text-sm md:text-base text-gray-900 whitespace-no-wrap">
                ...
                >
                  ...
                  <span className='text-green-500 font-bold'>Work done</span>
                )}
              </td>
            </tr>
          ))}
